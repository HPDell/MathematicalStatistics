...
\hsubparagraph{分布函数微分法} 先用定义求 $ Y $ 的分布函数 $ F_Y(y) $ 再求导得到密度函数 $ f_Y(y) $
...
\hparagraph{标准化随机变量} 若随机变量 $ X $ 有有限方差 $ DX > 0 $ ，则
$$ Y = \frac{X - EX}{DX} $$ 满足 $ EY = 0, DY = 1 $ 称为 $ X $ 的标准化随机变量。

\subsection{随机向量函数的分布} 

\hparagraph{离散型} 设 $ (X,Y) $ 是离散型随机变量，且概率函数为
...
\hsubparagraph{分布函数微分法} 设 $ (X,Y) $ 是二位连续性随机变量，其联合概率密度为 $ f(x,y) $ ，
则 $ Z = g(X,Y) $ 的分布函数为
