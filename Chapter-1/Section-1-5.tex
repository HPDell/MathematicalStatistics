...
\hparagraph{随机变量的数学期望} 设随机变量 $ X $ 的分布函数为 $ F_X(x) $ ，若积分
...
\hparagraph{随机变量函数的数学期望} 设随机变量 $ Y $ 时随机变量 $ X $ 的函数 $ Y = g(X) $ ，
...
\hparagraph{二维随机向量函数的数学期望} 设 $ (X,Y) $ 为二维随机向量，分布函数为 $ F_{X,Y}(x,y) $ ，
...
\hparagraph{方差} 设 $ X $ 是一个随机变量，若 $ E((X - E(X))^2) $ 存在，则称
$$ D(X) = E((X - E(X))^2) $$
是 $ X $ 的方差，方差的算术平方根 $ \sqrt{D(X)} = \sigma(X) $ 称为标准差。

\hsubparagraph{方差的含义} 方差刻划了随机变量的取值对于其数学期望的离散程度。

\hsubparagraph{方差的运算率}
...
\hparagraph{协方差} 设二维随机变量 $ (X,Y) $ 若 $ E((X - E(X))(Y - E(Y))) $ 存在，则称他为 $ X $ 与 $ y $ 的协方差，
记为 $ \Cov(X,Y) $ 。
\hsubparagraph{协方差的性质}
...
\hsubparagraph{协方差的含义} 协方差的大小在一定程度上反映了 $ X $ 和 $ Y $ 相互间的关系，
但它还受 $ X $ 与 $ Y $ 本身度量单位的影响。

\hparagraph{相关系数} 设 $ D(X) > 0, D(Y) > 0 $ 则称
\begin{equation}
...
\hparagraph{矩} 设 $ X $ 和 $ Y $ 是随机变量，有 $ k,l = 1,2,\cdots $ ，则

\hsubparagraph{（原点）矩} 若 $ E(X^k) $ 存在，则称它为 $ X $ 的 $ k $ 阶（原点）矩；
\hsubparagraph{中心矩} 若 $ E((X - E(X))^k) $ 存在，则称它为  $ X $ 的 $ k $ 阶中心矩；
\hsubparagraph{混合（原点）矩} 若 $ E(X^kY^l) $ 存在，则称它为 $ X $ 和 $ Y $ 的 $ k+l $ 阶混合（原点）矩；
\hsubparagraph{混合（原点）矩} 若 $ E((X - E(X))^k (Y - E(Y))^l) $ 存在，
则称它为 $ X $ 和 $ Y $ 的 $ k+l $ 阶混合中心矩；

\hparagraph{协方差矩阵} 若 $ n $ 维随机变量的 $ n^2 $ 个二阶中心矩都存在，将他们排列成矩阵形式，
...
\hparagraph{切比雪夫不等式} 对任意具有有限方差的随机变量 $ X $ ，都有
...
\hparagraph{A.L.Cauchy－Schwarz不等式} 设随机向量 $ (X,Y) $ 满足 $ E(X^2) < \infty, E(Y^2) < \infty $
则有 \begin{equation}
