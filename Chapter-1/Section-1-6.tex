...
\hparagraph{复随机变量} 设 $ X,Y $ 是概率空间 $ \{ \Omega, \mathcal{F}, P \} $
上的实随机变量，则 $ Z = X + \symrm{}{i}Y $ 称为复随机变量。

\hsubparagraph{独立性} 如果 $ (X_1,Y_1), (X_2,Y_2), \cdots, (X_n,Y_n) $ 互相独立，
就称复随机变量 $ X_1 + \symrm{i} Y_1, X_2 + \symrm{i} Y_2, \cdots, X_n + \symrm{i} Y_n $ 互相独立。

\hsubparagraph{期望} $ E(Z) = E(X) + \symrm{i} E(Y) $

\hparagraph{特征函数} 设 $ X $ 的分布函数为 $ F_X(x) $ 称
\begin{equation}
...
\hsubparagraph{存在性} 因为 $ e^{\symrm{i}tx} = \cos tx + \symrm{i} \sin tx $
...
\hparagraph{多元特征函数} 若随机向量 $ (\xi_1, \xi_2, \cdots, \xi_n) $ 的分布函数为 $ F(x_1, x_2, \cdots, x_n) $ ，
特征函数为
\begin{equation}
